...
class Posting < ActiveRecord::Base
  ...
  scope :with_from_address,   lambda { |value| where("from_address LIKE ? and from_address LIKE ?", "%#{Posting.format(value).titleize}%", "%#{(value.split & (DISTRICTS + NEIGHBORHOODS.collect(&:last))).first}%") if value }
  scope :with_to_address,     lambda { |value| where("to_address LIKE ? and to_address LIKE ?", "%#{Posting.format(value).titleize}%", "%#{(value.split & (DISTRICTS + NEIGHBORHOODS.collect(&:last))).first}%") if value }
